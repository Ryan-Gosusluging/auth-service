...
import (
	...
	"github.com/Ryan-Gosusluging/auth-service/internal/entity"
	"github.com/Ryan-Gosusluging/auth-service/internal/repository"
)
...
func (r *UserRepository) SaveUser(ctx context.Context, user *entity.User) (int64, error) {
	...
	err := r.db.QueryRowContext(ctx, query, user.Email, user.Password, user.Role, user.Status).Scan(&user.ID)
	if err != nil {
		return 0, err
	}
	return user.ID, nil
